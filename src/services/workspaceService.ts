...
export async function createWorkspace(name: string, creatorId: string): Promise<Workspace> {
  ...
  logInfo(`Workspace created: ${name} by ${creatorId}`);
  return workspace!;
}
...
export async function getWorkspaceById(workspaceId: string): Promise<Workspace & { members: WorkspaceMember[] } | null> {
  ...
  const membersResult = await query<WorkspaceMember & { email: string; full_name: string | null }>(
    `SELECT wm.*, u.email, u.full_name FROM workspace_members wm JOIN users u ON u.id = wm.user_id WHERE wm.workspace_id = $1`,
    [workspaceId]
  );
...
export async function getUserWorkspaces(userId: string): Promise<Workspace[]> {
  const result = await query<Workspace>(
    `SELECT w.* FROM workspaces w JOIN workspace_members wm ON wm.workspace_id = w.id WHERE wm.user_id = $1 ORDER BY w.created_at DESC`,
    [userId]
  );
  return result.rows;
}
...
export async function removeWorkspaceMember(workspaceId: string, userId: string): Promise<void> {
  await transaction(async (client) => {
    ...
    await client.query(
      `DELETE FROM project_memberships WHERE user_id = $1 AND project_id IN (SELECT id FROM projects WHERE workspace_id = $2)`,
      [userId, workspaceId]
    );
  });
}
